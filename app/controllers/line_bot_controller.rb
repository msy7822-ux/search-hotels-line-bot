class LineBotController < ApplicationController
  # FSRF攻撃のリスクがほとんどない場合に、CSRF対策を無効化するコード
  protect_from_forgery except: [:callback]

  # callbackメソッドは、Lineとのメッセージなどのデータの送受信を行う
  def callback
    ...
    # 送信されたメッセージボディがテキストメッセージかどうかを解析する
  ...
  def search_and_create_message(keyword)
    ...
    # getメソッドは、urlとqueryを引数に渡してレスポンスを受け取る
    response = http_client.get(url, query)
    # responseをJSON型にパースする
    response = JSON.parse(response.body)

    if response.key?('error')
      text = "この検索条件に該当する宿泊施設は見つかりませんでした。\n条件を変えて再検索してください。"
      message = {
        type: 'text',
        text: text
      }
